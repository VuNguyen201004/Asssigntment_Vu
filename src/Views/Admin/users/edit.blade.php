...
@section('content') <h1>Cập nhật người dùng: {{ $user['name'] }}
    ...
    <form action="{{ url("admin/users/{$user['id']}/update") }}" enctype="multipart/form-data" method="POST">
        ...
        <div class="mb-3 mt-3">
            ...
            <input type="radio" class="form-radio" id="type_admin" value="admin" @if ($user['type'] == 'admin') checked @endif name="type">
            <label for="type_admin" class="form-label">Admin</label>

            <input type="radio" class="form-radio" id="type_member" value="member" @if ($user['type'] == 'member') checked @endif name="type">
